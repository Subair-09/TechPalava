  ...
  return (
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        <MetricCard label="Total Site Views" value="1.42M" change="12.4%" isPositive={true} icon={<svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>} />
        <MetricCard label="Unique Visitors" value="842.1k" change="8.2%" isPositive={true} icon={<svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>} />
        <MetricCard label="Avg Read Time" value="4m 32s" change="1.4%" isPositive={false} icon={<svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
        <MetricCard label="Articles Live" value="128" change="12%" isPositive={true} icon={<svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10l4 4v10a2 2 0 01-2 2z" /></svg>} />
        <MetricCard label="Active Jobs" value="42" change="24.1%" isPositive={true} icon={<svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745" /></svg>} />
        <MetricCard label="Ad Revenue" value="$12.4k" change="4.1%" isPositive={true} icon={<svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        <div className="lg:col-span-1 bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm h-full flex flex-col">
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50/50">
              <tr>
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Avg Engagement</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {[
                { title: "Apple Vision Pro: Year One Verdict", views: "42,102", engagement: "6m 12s" },
                { title: "The Sovereign AI Race in Africa", views: "38,592", engagement: "8m 45s" },
                { title: "NVIDIA Blackwell vs. The World", views: "31,042", engagement: "4m 20s" }
              ].map((row, i) => (
                <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-8 py-5 font-bold text-slate-900">{row.title}</td>
                  <td className="px-8 py-5 text-right font-mono text-xs font-bold text-slate-600">{row.views}</td>
                  <td className="px-8 py-5 text-right text-xs font-bold text-slate-900">{row.engagement}</td>
                </tr>
              ))}
            </tbody>
          </table>
